  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Reception Check-in</h2>
        {step > 1 && step < 6 && (
           <button onClick={resetFlow} className="text-sm font-semibold text-gray-500 hover:text-red-500 flex items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="glass-card p-6 border-l-4 border-blood-500">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold text-lg text-gray-800">Scan QR Code</h3>
              <button 
            ...
            {showCamera ? (
              <div className="mb-4">
                ...
                <p className="text-sm text-center text-gray-500 mt-4">Point your camera at the donor's QR code</p>
              </div>
            ) : (
              <div className="flex flex-col space-y-3 mt-8">
                <label className="text-sm font-medium text-gray-600">Manual QR Token Entry</label>
          ...
          <div className="glass-card p-6 flex flex-col justify-center items-center text-center">
             ...
             <h3 className="font-bold text-gray-800 mb-2">Check-in Process</h3>
             <p className="text-gray-500 text-sm">Scan an appointment QR code to begin the verification workflow. The system enforces a strict 3-hour arrival window.</p>
          </div>
        </div>
      )}
      ...
      {step === 2 && scannedAppointment && (
        <div className="glass-card p-6 relative">
          <h3 className="font-bold text-xl text-gray-800 border-b pb-3 mb-6 flex items-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <label className="border-2 border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors group">
                ...
                <span className="text-xs text-gray-500 text-center max-w-xs truncate block">
            ...
            {idMatchError && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-6 h-fit shadow-sm">
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {idMatchError.mismatchedParts.map((part, idx) => (
                           <span key={idx} className="bg-red-100 text-red-800 px-2 py-1 rounded text-xs font-bold font-mono">"{part}"</span>
                        ))}
                      </div>
                    </div>
                  )}
                    ...
                    <div className="flex flex-col mt-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {idMatchError.matchedParts.map((part, idx) => (
                           <span key={idx} className="bg-green-100 text-green-800 px-2 py-1 rounded text-xs font-bold font-mono">"{part}"</span>
                        ))}
                      </div>
                    </div>
                  )}
                ...
                {idMatchError.extractedText !== undefined && (
                  <div className="mt-4 p-3 bg-white rounded border border-red-100">
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1 block">Raw Text Read by OCR:</span>
                    <p className="text-xs font-mono text-gray-600 break-words whitespace-pre-wrap max-h-32 overflow-y-auto">
                      {idMatchError.extractedText || "(No text could be extracted. Is the image blurry?)"}
                    </p>
                  </div>
                )}
                
                <button onClick={() => { setIdFile(null); setIdMatchError(null); }} className="mt-6 text-red-600 font-bold text-sm hover:text-red-800 hover:underline">
            ...
            {isIdUploaded && (
              <div className="bg-green-50 border border-green-200 rounded-xl p-8 flex flex-col items-center justify-center text-center shadow-sm">
                <CheckCircle className="w-16 h-16 text-green-500 mb-4" />
                <h4 className="font-bold text-green-800 text-xl mb-2">Document Uploaded Successfully</h4>
                <p className="text-green-600 text-sm mb-8">The identity document has been securely attached to the visit record.</p>
                ...
                <button onClick={() => { setIsIdUploaded(false); setIdFile(null); }} className="mt-4 text-gray-500 font-bold text-sm hover:text-gray-700">
      ...
      {step === 3 && (
        <div className="glass-card p-6 relative">
          ...
          <p className="text-sm text-gray-500 mb-6 bg-yellow-50 p-3 rounded-lg border border-yellow-200 text-yellow-800">
            ...
            <div className="space-y-4">
              <div>
                ...
                <label className="border-2 border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors group">
                  ...
                  <span className="text-xs text-gray-500 mt-1 max-w-xs truncate block">
                ...
                <div key={q.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border">
                   ...
                   <div className="flex space-x-2">
                      <button 
                        ...
                        className={`px-4 py-1.5 rounded-full text-sm font-bold transition-colors ${answers[q.id as keyof typeof answers] === true ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-600'}`}
                      >YES</button>
                      <button 
                        onClick={() => setAnswers({...answers, [q.id]: false})}
                        className={`px-4 py-1.5 rounded-full text-sm font-bold transition-colors ${answers[q.id as keyof typeof answers] === false ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-600'}`}
                      >NO</button>
                   </div>
                </div>
             ))}
      ...
      {step === 6 && assignedQueue && (
        <div className="flex flex-col items-center max-w-4xl mx-auto space-y-6">
           ...
           <div className="w-full flex justify-end space-x-4 print:hidden">
              <button onClick={() => window.print()} className="glass-button px-6 py-2 flex items-center bg-blue-50 text-blue-700 hover:bg-blue-100 border-blue-200">
                <Printer className="w-5 h-5 mr-2" /> Print Form
              </button>
              <button onClick={resetFlow} className="glass-button px-6 py-2 bg-green-50 text-green-700 hover:bg-green-100 border-green-200">
           ...
           <div id="printable-form" className="bg-white w-full shadow-2xl rounded-sm p-10 print:p-2 relative overflow-hidden border border-gray-200 print:shadow-none print:border-none print:m-0 flex flex-col justify-between">
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="flex justify-between items-start border-b-4 border-gray-800 pb-4 mb-4">
                   <div className="flex items-center space-x-4">
                      ...
                      <div>
                         <h1 className="text-2xl font-black text-gray-900 tracking-tight uppercase">BLOODLINK NETWORK</h1>
                         <h2 className="text-md font-bold text-gray-600 uppercase tracking-widest mt-1">Donor Registration & Consent Form</h2>
                         <p className="text-sm text-gray-500 mt-1">Center: {assignedQueue.visit?.appointment?.bloodBank?.name || "BloodLink Hub"}</p>
                      </div>
                   </div>
                   <div className="flex flex-col items-end text-right">
                      ...
                      <p className="text-xs font-mono text-gray-500 mt-1">{assignedQueue.queueNumber}</p>
                   </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                   <div className="space-y-3">
                      <div className="flex border-b border-gray-200 pb-1">
                        <span className="w-32 text-xs font-bold text-gray-500 uppercase tracking-wider">Queue No:</span>
                        <span className="font-black text-lg text-gray-900">{assignedQueue.queueNumber}</span>
                      </div>
                      <div className="flex border-b border-gray-200 pb-1">
                        <span className="w-32 text-xs font-bold text-gray-500 uppercase tracking-wider">Room / Counter:</span>
                        <span className="font-bold text-gray-900">{assignedQueue.room?.roomNumber || "-"} / {assignedQueue.counter?.counterNumber || "-"}</span>
                      </div>
                      <div className="flex border-b border-gray-200 pb-1">
                        <span className="w-32 text-xs font-bold text-gray-500 uppercase tracking-wider">Appt Date:</span>
                        <span className="font-bold text-gray-900">{assignedQueue.visit?.appointment?.date ? new Date(assignedQueue.visit.appointment.date).toLocaleDateString() : "-"} at {assignedQueue.visit?.appointment?.timeSlot || "-"}</span>
                      </div>
                   </div>
                   
                   <div className="space-y-3">
                      <div className="flex border-b border-gray-200 pb-1">
                        <span className="w-32 text-xs font-bold text-gray-500 uppercase tracking-wider">Visit ID:</span>
                        <span className="font-bold font-mono text-sm text-gray-900">{assignedQueue.visit?.visitNumber || "-"}</span>
                      </div>
                      <div className="flex border-b border-gray-200 pb-1">
                        <span className="w-32 text-xs font-bold text-gray-500 uppercase tracking-wider">Appointment ID:</span>
                        <span className="font-bold font-mono text-sm text-gray-900">{assignedQueue.visit?.appointment?.id || "-"}</span>
                      </div>
                      <div className="flex border-b border-gray-200 pb-1">
                        <span className="w-32 text-xs font-bold text-gray-500 uppercase tracking-wider">Registration:</span>
                        <span className="font-bold text-gray-900">{assignedQueue.createdAt ? new Date(assignedQueue.createdAt).toLocaleString() : "-"}</span>
                      </div>
                   </div>
                </div>

                {/* DONOR DEMOGRAPHICS SECTION */}
                <div className="bg-gray-50 print:bg-white border border-gray-300 rounded-lg p-4 mb-6">
                   <h3 className="text-sm font-black text-gray-800 uppercase tracking-widest border-b border-gray-300 pb-2 mb-3">1. Donor Information</h3>
                   
                   <div className="grid grid-cols-2 gap-x-8 gap-y-3">
                      <div className="flex flex-col border-b border-gray-200 pb-1">
                        <span className="text-[10px] font-bold text-gray-500 uppercase">Full Name</span>
                        <span className="font-bold text-gray-900 text-lg uppercase">{assignedQueue.visit?.appointment?.donor?.user?.name || "-"}</span>
                      </div>
                      <div className="flex flex-col border-b border-gray-200 pb-1">
                        <span className="text-[10px] font-bold text-gray-500 uppercase">Donor ID</span>
                        <span className="font-mono font-bold text-gray-900">{assignedQueue.visit?.appointment?.donor?.id || "-"}</span>
                      </div>
                      
                      <div className="flex justify-between border-b border-gray-200 pb-1">
                        <div className="flex flex-col">
                           <span className="text-[10px] font-bold text-gray-500 uppercase">Age</span>
                           <span className="font-bold text-gray-900">{assignedQueue.visit?.appointment?.donor?.age || "-"} Yrs</span>
                        </div>
                        <div className="flex flex-col">
                           <span className="text-[10px] font-bold text-gray-500 uppercase">Gender</span>
                           <span className="font-bold text-gray-900">{assignedQueue.visit?.appointment?.donor?.gender || "-"}</span>
                        </div>
                        <div className="flex flex-col">
                      ...
                      <div className="flex justify-between border-b border-gray-200 pb-1">
                        <div className="flex flex-col">
                           ...
                           <span className="font-bold text-gray-900">{assignedQueue.visit?.appointment?.donor?.mobileNumber?.replace(/.(?=.{4})/g, '*') || "Not Provided"}</span>
                        </div>
                        <div className="flex flex-col text-right">
                           <span className="text-[10px] font-bold text-gray-500 uppercase">Identity Verified</span>
                           <span className="font-bold text-gray-900">Yes (Aadhaar/PAN)</span>
                        </div>
                      </div>

                      <div className="col-span-2 flex flex-col border-b border-gray-200 pb-1">
                        <span className="text-[10px] font-bold text-gray-500 uppercase">Residential Address</span>
                        <span className="font-bold text-gray-900">{assignedQueue.visit?.appointment?.donor?.address || "Address not specified"}</span>
                      </div>
                   </div>
                </div>
                ...
                <div className="border border-gray-800 rounded-lg p-4 mb-6">
                   <h3 className="text-sm font-black text-gray-800 uppercase tracking-widest border-b border-gray-800 pb-2 mb-3">2. Medical Examination (For Staff Use Only)</h3>
                   <div className="grid grid-cols-4 gap-4">
                      <div className="border-b border-dashed border-gray-400 pb-1 flex flex-col">
                         <span className="text-xs text-gray-600 mb-4">Weight (kg)</span>
                      </div>
                      <div className="border-b border-dashed border-gray-400 pb-1 flex flex-col">
                         <span className="text-xs text-gray-600 mb-4">Blood Pressure (mmHg)</span>
                      </div>
                      <div className="border-b border-dashed border-gray-400 pb-1 flex flex-col">
                         <span className="text-xs text-gray-600 mb-4">Hemoglobin (g/dL)</span>
                      </div>
                      <div className="border-b border-dashed border-gray-400 pb-1 flex flex-col">
                         <span className="text-xs text-gray-600 mb-4">Pulse (bpm)</span>
                      </div>
                   </div>
                   <div className="mt-4 flex items-end space-x-4">
                      <span className="text-xs font-bold uppercase text-gray-800">Donor Fit for Donation?</span>
                      <div className="flex space-x-4">
                      ...
                      <span className="text-[10px] text-gray-500">Medical Officer Signature</span>
                   </div>
                </div>

                {/* DECLARATION AND CONSENT */}
                <div className="mb-6">
                   <h3 className="text-sm font-black text-gray-800 uppercase tracking-widest border-b border-gray-300 pb-1 mb-2">3. Declaration & Consent</h3>
                ...
                <div className="mt-auto pt-6 border-t-2 border-gray-800 grid grid-cols-3 gap-6">
                   <div className="flex flex-col items-center pt-12 border-t border-dashed border-gray-400">
                      <span className="text-xs font-bold uppercase text-gray-800">Signature of Donor</span>
                      <span className="text-[10px] text-gray-500 mt-1">Date: ___ / ___ / 20__</span>
                   </div>
                   <div className="flex flex-col items-center pt-12 border-t border-dashed border-gray-400">
                      <span className="text-xs font-bold uppercase text-gray-800">Signature of Phlebotomist</span>
                      <span className="text-[10px] text-gray-500 mt-1">Staff ID: ________________</span>
                   </div>
                   <div className="flex flex-col items-center pt-12 border-t border-dashed border-gray-400">
                      <span className="text-xs font-bold uppercase text-gray-800">Bank / Center Seal</span>
                   </div>
                </div>
      ...
      <div className="glass-card p-6 mt-8">
        <h3 className="font-semibold mb-4 text-gray-800 border-b pb-2 flex justify-between">
           <span>Today's Queue Dashboard</span>
           <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full">{queue.length} Active</span>
        </h3>
        {queue.length === 0 ? (
           <p className="text-gray-500 italic py-4 text-center">No donors checked in yet.</p>
        ) : (
               ...
               <tbody>
                 {queue.map((q: any) => (
                   <tr key={q.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors">
                     ...
                     <td className="p-3 font-medium text-gray-800">{q.visit?.appointment?.donor?.user?.name || "Unknown"}</td>
