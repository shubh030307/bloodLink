...
export default function LabReportReview() {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Loading laboratory report...</div>;
  if (!unit || !unit.labReport) return <div className="p-8 text-center text-red-500">Report not found.</div>;
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6 pb-24">
      ...
      <div className="bg-white dark:bg-gray-900 shadow-xl ring-1 ring-gray-200 dark:ring-gray-800 rounded-lg overflow-hidden mx-auto max-w-4xl relative">
        ...
        <div className="p-10 space-y-8">
          ...
          {/* Header Section */}
          <div className="flex justify-between items-start border-b-2 border-gray-100 dark:border-gray-800 pb-6">
            <div className="flex items-center gap-4">
              <img src="/favicon.jpeg" alt="BloodLink Logo" className="w-16 h-16 object-contain rounded-lg shadow-sm" />
              <div>
                <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight uppercase">BloodLink</h2>
                <h3 className="text-sm font-bold text-red-600 tracking-widest uppercase">National Reference Laboratory</h3>
                <p className="text-xs text-gray-500 mt-1">123 Health Avenue, Medical District • +1 (800) 555-BLNK</p>
              </div>
            </div>
            <div className="text-right">
              <h1 className="text-xl font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-1">Official Lab Report</h1>
              <p className="text-sm font-mono text-gray-600 dark:text-gray-400 font-medium bg-gray-100 dark:bg-gray-800 inline-block px-2 py-0.5 rounded">{unit.labReport.reportNumber}</p>
              <p className="text-xs text-gray-500 mt-2 font-medium">Generated: {new Intl.DateTimeFormat('en-US', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(unit.labReport.generatedAt))}</p>
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg border border-gray-100 dark:border-gray-800">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 border-b border-gray-200 dark:border-gray-700 pb-2">Unit Details</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Unit Number:</span> <span className="font-mono font-medium text-gray-900 dark:text-white">{unit.unitNumber}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Component:</span> <span className="font-medium text-gray-900 dark:text-white">{unit.component}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Blood Group:</span> <span className="font-black text-red-600 text-lg leading-none">{unit.bloodGroup}</span></div>
              </div>
            </div>
            
            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg border border-gray-100 dark:border-gray-800">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 border-b border-gray-200 dark:border-gray-700 pb-2">Collection Source</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Collection Center:</span> <span className="font-medium text-gray-900 dark:text-white">{unit.collectionCenter?.name}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Collection Date:</span> <span className="font-medium text-gray-900 dark:text-white">{new Intl.DateTimeFormat('en-US', { dateStyle: 'medium' }).format(new Date(unit.donation.collectionDate))}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Source Sticker ID:</span> <span className="font-mono font-medium text-gray-900 dark:text-white">{unit.label?.stickerId}</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4">
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="border-b-2 border-gray-900 dark:border-gray-100 text-gray-900 dark:text-white">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {results?.map((res, idx) => (
                  <tr key={idx} className="group hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="py-4 font-semibold text-gray-900 dark:text-white">{res.test?.testName || res.testId}</td>
                    <td className="py-4 font-mono text-gray-600 dark:text-gray-300">{res.resultValue}</td>
                    <td className="py-4">
                      {res.resultStatus === 'NORMAL' || res.resultStatus === 'NEGATIVE' ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800/50">
                          <CheckCircle className="w-3 h-3" /> {res.resultStatus}
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-red-50 text-red-700 border border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800/50">
                          <AlertTriangle className="w-3 h-3" /> {res.resultStatus}
                        </span>
                      )}
                    </td>
                    <td className="py-4 text-gray-500 text-xs max-w-[200px] truncate" title={res.remarks || ''}>
                      {res.remarks || '--'}
                    </td>
                  </tr>
                ))}
          ...
          {/* Signatures & Footer */}
          <div className="pt-12 pb-4 mt-8 border-t-2 border-gray-100 dark:border-gray-800 grid grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <div className="border-t border-gray-400 mt-2 pt-2">
                <p className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">Testing Performed By</p>
                <p className="text-xs text-gray-500">Automated Equipment</p>
              </div>
            </div>
            <div className="text-center">
              {/* Spacer */}
            ...
            <div className="text-center">
              ...
              <div className="border-t border-gray-400 mt-2 pt-2">
                <p className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">Authorized Reviewer</p>
      ...
      {showReject && (
        <form onSubmit={handleReject} className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/50 rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <select 
                ...
                className="w-full px-4 py-2 border border-red-200 dark:border-red-800 rounded-lg bg-white dark:bg-gray-800 focus:ring-2 focus:ring-red-500"
              >
          ...
          <div className="flex gap-3">
            {!showReject && (
              <button 
                ...
                disabled={actionLoading}
                className="px-6 py-2.5 bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400 font-medium rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors flex items-center gap-2"
              >
